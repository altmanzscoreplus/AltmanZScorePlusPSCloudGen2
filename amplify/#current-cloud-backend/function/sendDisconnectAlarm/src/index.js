...
async function sendAlarm(deviceId, device, contact, alarmLevel, localTime) {
    ...
    const subjectMsg = `PowerSight Remote Monitoring Alarm: ${generateDeviceLabel(device)}`
    const bodyMsg = `${updatedMessage} Last communication was on ${localTime}
Cell: ${deviceId}`

    if (contact.alarm_level_email === alarmLevel) await sendEmail(contact.email, subjectMsg, bodyMsg);
    if (contact.alarm_level_sms === alarmLevel) await sendEmail(contact.email, subjectMsg, bodyMsg); // await sendSMS(contact.phone, `${subjectMsg} - ${bodyMsg});
    if (contact.alarm_level_phone === alarmLevel) await sendEmail(contact.email, subjectMsg, bodyMsg); // await sendVoiceMessage(contact.phone, `${subjectMsg} - ${bodyMsg}`);
}
        ...
        if (gatewayResult.Items.length > 0) {
            ...
            if (gateway.active_inactive_status === "Active" && gateway.allocated_unallocated_status === "Allocated") {
                gateway = {
                    ...
                    room_location: gateway.room_location,
                    site_location: gateway.site_location
                };
        ...
        if (analyzerResult.Items.length > 0) {
            ...
            if (analyzer.active_inactive_status === "Active" && analyzer.assigned_unassigned_status === "Assigned") {
                analyzer = {
                    ...
                    site_location: analyzer.site_location,
...
function generateDeviceLabel(device) {
  const priorityFields = [
    { key: 'circuit', label: device.circuit },
    { key: 'room_location', label: device.room_location },
    { key: 'site_location', label: device.site_location },
    { key: 'serial_number', label: device.serial_number },
    { key: 'model', label: device.model }
  ];

  // Filter out fields that are present and non-empty
  const availableFields = priorityFields.filter(field => field.label);

  // Take the top 2 fields based on priority
  const topTwo = availableFields.slice(0, 2);

  if (topTwo.length < 2) {
    return `Device`; // Default fallback
  }

  const keys = topTwo.map(f => f.key);
  const isModelSerialOnly =
    keys.includes('model') &&
    keys.includes('serial_number') &&
    topTwo.length === 2;

  if (isModelSerialOnly) {
    const model = topTwo.find(f => f.key === 'model').label;
    const serial = topTwo.find(f => f.key === 'serial_number').label;
    return `${model}-${serial}`;
  }

  // Reverse the order: lower-priority field first
  return `${topTwo[1].label} – ${topTwo[0].label}`;
}