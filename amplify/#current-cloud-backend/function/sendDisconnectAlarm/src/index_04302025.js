        ...
        if (deviceData.gateway) {
            ...
            if (adminAlarm && timeDifference >= adminAlarm.interval) {
                ...
                for (let contact of adminContacts) {
                    ...
                    await sendAlarm(deviceId, contact, adminAlarm.level, localTime);
...
async function getAlarmInterval(table, id) {
    console.log(`gatAlarmInterval ${table} ${id}`);
    try {
...
async function sendAlarm(deviceId, contact, alarmLevel, localTime) {
    console.log(`sendAlarm deviceId ${deviceId} contact email ${contact.email} phone ${contact.phone} alarm level ${alarmLevel}`)

    const message = getAlarmMessage(alarmLevel);
    console.log(`message ${message}`);

    if (contact.alarm_level_email === alarmLevel) await sendEmail(deviceId, contact.email, `${message} at ${localTime}`);
    //if (contact.alarm_level_sms === alarmLevel) await sendSMS(contact.phone, `Alarm ${alarmLevel} : Device ${deviceId}  - ${message} at ${localTime}`);
    //if (contact.alarm_level_phone === alarmLevel) await sendVoiceMessage(contact.phone, `Alarm ${alarmLevel} : Device ${deviceId} - ${message} at ${localTime}`);
}


// Function to send an email
async function sendEmail(deviceId, recipientEmail, message) {
    ...
    const params = {
        ...
        Message: {
            ...
            Body: { Text: { Data: `Device ${deviceId} -  ${message}` } }
        }
    };
