...
exports.handler = async (event) => {
	...
	// Delete data from Customer Table
		...
		console.log('deleting customer record');
			...
			await updateGatewayStatus(body.customerId,tableSuffix);
                	console.log(`Updated Gateway allocation status, assignment status and customer_id`);
			await updateAnalyzerStatus(body.customerId,tableSuffix);
                	console.log(`Updated Analyzer allocation status, assignment status and customer_id`);
		}
		catch (error)
		{
...
async function updateGatewayStatus(customerId,tableSuffix) {
  const tableName = `Gateway${tableSuffix}`; // Replace with your table name

  // Scan for items with the given customer_id
  const scanParams = {
    TableName: tableName,
  ...
  try {
    ...
    for (const item of items) {
      const updateParams = {
        TableName: tableName,
        Key: {
          'id': item.id // Use 'id' as the primary key
        },
        UpdateExpression: 'SET allocated_unallocated_status = :unallocated, assigned_unassigned_status = :unassigned, customer_id = :null, client_id = :clientnull',
        ExpressionAttributeValues: {
          ':unallocated': 'Unallocated',
          ':unassigned': 'Unassigned',
          ':null': '0000-0000-0000-0000-0000-0000-0000-0000',
          //':clientnull': null
          ':clientnull': '0000-0000-0000-0000-0000-0000-0000-0000'
        }
      };
    ...
    console.log(`${items.length} item(s) were updated`);

  } catch (err) {
    console.error('Error updating the Gateway status:', err);
  }
}

async function updateAnalyzerStatus(customerId,tableSuffix) {
  const tableName = `Analyzer${tableSuffix}`; // Replace with your table name
  ...
  try {
    ...
    console.error('Error updating the Analyzer status:', err);
  }
}
