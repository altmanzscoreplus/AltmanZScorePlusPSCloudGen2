...
import { DeviceAllocationListTable } from 'src/sections/dashboard/datadevice/device-allocation-list-table';
import { DeviceAllocationTable } from 'src/sections/dashboard/datadevice/device-allocation-table';
...
const Page: NextPage = () => {
  ...
  useEffect(() => {
    ...
    if (selecteddata?.analyzer_rental?.items?.filter((gat) => !gat.termination_date).length > 0) {
      ...
      setCompanyValue(
        selecteddata.analyzer_rental.items.filter((gat) => !gat.termination_date)[0].customer?.name
      );
    } else {
      setCompanyValue('');
      customersSearch.handleFiltersChange({
    ...
    if (id) {
      try {
        ...
        if (assets.data?.analyzer_rental?.items.filter((gat) => !gat.termination_date).length > 0) {
          ...
          setCompanyValue(
            assets.data.analyzer_rental.items.filter((gat) => !gat.termination_date)[0].customer
              ?.name
          );
        } else {
          // setCompanyValue('');
          ...
          <Stack spacing={1}>
            <Stack spacing={1}>
              ...
              >
                ...
                >
                  <Stack spacing={1}>
                    <Typography variant="h4">Analyzer Allocation</Typography>
                  </Stack>
                </Stack>
              </Stack>
            </Stack>
