...
interface GatewayAllocationTablecustomerProps {
  count?: number;
  items?: Customer[];
  onDeselectAll?: () => void;
...
export const GatewayAllocationTablecustomer: FC<GatewayAllocationTablecustomerProps> = (props) => {
  const {
    count = 0,
    items = [],
    onDeselectAll,
  ...
  const updateallocation = async (id: any) => {
    ...
    try {
      // Update Gateway
      console.log(gatewaydetail, 'gatewaydetailsssssssssssssssssssssssss');

      const updateGatewayResponse = await API.graphql(
        graphqlOperation(mutations.updateGateway, {
          input: { customer_id: id, id: gatewayid, allocated_unallocated_status: 'Allocated', communication_status: "Archive" },
        })
      );
...
GatewayAllocationTablecustomer.propTypes = {
