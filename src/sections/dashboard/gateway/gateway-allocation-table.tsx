...
interface GatewayAllocationTableProps {
  count?: number;
  items?: Customer[];
  onDeselectAll?: () => void;
...
export const GatewayAllocationTable: FC<GatewayAllocationTableProps> = (props) => {
  const {
    count = 0,
    items = [],
    onDeselectAll,
    ...
    try {
      ...
      const updateGatewayResponse = await API.graphql(
        graphqlOperation(mutations.updateGateway, {
          input: { customer_id: id, id: selecteddata?.id,allocated_unallocated_status:"Allocated", communication_status: "xx"},
        })
      );
  ...
  const updateunallocation = async (id: any) => {
    try {
      ...
      const updateGatewayResponse = await API.graphql(
        graphqlOperation(mutations.updateGateway, {
          input: { customer_id: null, id: selecteddata?.id,allocated_unallocated_status:"Unallocated", communication_status: "Not_Detected"},
        })
      );
          ...
          <TableBody>
            ...
            {items?.items
              ?.filter((customer) => {
                if (
                  !companyValue &&
                  customer?.gateway_rental?.items?.some((item) => !item.termination_date === null)
                ) {
                  return false;
                }
...
GatewayAllocationTable.propTypes = {
