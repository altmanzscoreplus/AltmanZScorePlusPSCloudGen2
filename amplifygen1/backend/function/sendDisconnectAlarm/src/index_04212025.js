...
//	sendVoiceMessage('+19739440408', 'Test');
	...
	if (deviceData.gateway) {
		    console.log("Processing gateway logic...");
                    let { client_id, customer_id, device_type, enable_or_disable_alarm, id } = deviceData.gateway;
	...
	if (deviceData.analyzer) {
		    console.log("Processing analyzer  logic...");
                    let { client_id, customer_id, device_type, enable_or_disable_alarm, id } = deviceData.analyzer ;
...
async function sendAlarm(deviceId, contact, alarmLevel) {
    ...
    if (contact.alarm_level_sms === alarmLevel) await sendSMS(contact.phone, `Alarm ${alarmLevel} : Device ${deviceId} Disconnected`);
    if (contact.alarm_level_phone === alarmLevel) await sendVoiceMessage(contact.phone, `Alarm ${alarmLevel} : Device ${deviceId} Disconnected`);
}
