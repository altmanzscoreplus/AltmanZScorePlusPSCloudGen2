...
exports.handler = async (event) => {
    ...
    for (const record of event.Records) {
        ...
        let adminAlarm = await getAdminAlarmInterval();
        if (adminAlarm && timeDifference >= adminAlarm.interval) {
            console.log(`Admin alarm threshold breached for device ${deviceId}, sending admin alarms.`);
            ...
            await logAlarmSent(deviceId, currentTime, "Admin");
        }
        ...
        await logAlarmSent(deviceId, currentTime, "Client");
    }
};
...
/** FUNCTION TO GET ADMIN ALARM INTERVAL **/
async function getAdminAlarmInterval() {
    try {
        const result = await dynamoDB.scan({ TableName: ADMIN_ALARM_LEVEL_TABLE }).promise();
        return result.Items.length > 0 ? { interval: result.Items[0].alarm_interval, level: result.Items[0].alarm_level } : null;
    } catch (error) {
        console.error("Error checking Admin alarm levels:", error);
        return null;
    }
}
...
async function sendAlarm(deviceId, contact, alarmLevel) {
    ...
    if (contact.alarm_level_phone === alarmLevel) await sendSMS(contact.phone, `Alert: Device ${deviceId} Disconnected`);
    if (contact.alarm_level_sms === alarmLevel) await sendVoiceMessage(contact.phone, `Alert: Device ${deviceId} Disconnected`);
}
...
async function logAlarmSent(deviceId, timestamp, recipient) {
    const params = {
        ...
        Item: {
            ...
            alarm_delivery_method: "EMAIL/SMS/VOICE",
            alarm_recipient: recipient
        }
    };
